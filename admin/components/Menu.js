...
const Menu = ({ isActive,
    index, menu, handleInfo, submitInfo }) => {
    const comment = menu.comments.join('\n');
	return (
		<>
			<li className={"item " + ((isActive === true) ? "on" : "")}>
				<img className="icon" src={`${process.env.NEXT_PUBLIC_IMAGE_SERVER}/view/${menu.path}.png`} alt="icon" />
                <p>
                    <input
                        type="text"
                        name="path"
                        value={menu.path}
                        onChange={handleInfo.bind(this, index)}
