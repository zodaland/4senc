...
const style = css`
    ...
    .title { position: absolute; top: 135px; left: 50px; }
`

const Header = (({menus, scroll, urlPath,
    info, handleInfo, submitInfo}) => {
    return(
		<>
        <header className={scroll && "scroll"}>
            ...
            <div className={"logo-wrap" + (scroll ? " none" : "")}>
				<img
                    className="logo"
                    src={`${process.env.NEXT_PUBLIC_IMAGE_SERVER}/view/logo.png`}
                    alt="logo"
                />
            </div>
			</Link>
            <input
                className={"title" + (scroll ? " none" : "")}
                name="title"
                value={info.title}
                onChange={handleInfo}
